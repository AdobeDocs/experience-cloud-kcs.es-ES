...
3. Elimine todos los nodos corruptos identificados en la lista de salida del último comando utilizando `rmNodes.groovy`.
   ...
   Donde la ruta del nodo corrupto es la ruta obtenida en el paso 2, por ejemplo: `/content/dam/test.txt/jcr:content/renditions/original/jcr:content/`. Nota: Al utilizar `oak-run.jar` versión 1.6.13 y posteriores, establecer `--read-write` Parámetro JVM si se produce un error como:
