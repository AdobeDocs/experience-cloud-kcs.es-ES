...
description: Aprenda a tomar volcados de hilos de una JVM en varios sistemas operativos.
...
Un volcado de hilos es una lista de todos los hilos Java que están activos actualmente en una máquina virtual Java (JVM). Este artículo analiza las varias formas de tomar volcados de hilos de una JVM.
...
- En Linux y UNIX, debe ejecutar el comando como el usuario propietario del proceso java:<br>\
...
- Si el proceso java no responde, a veces puede resultar útil utilizar la opción <b>-J-d64</b> (en sistemas de 64 bits), por ejemplo:<br>\
  `jstack -J-d64 -l   <pid>   >> threaddumps.log`
- Si el comando jstack (`jstack -l <pid> >> threaddumps.log`) genera el error `[` 1`]`  a continuación, ejecute el comando como el usuario propietario del proceso java. Por ejemplo:<br>\
...
1. Ejecute este comando para hacerlo:<br>\
   `kill -QUIT <pid>`
...
2. Inicie la JVM con estos tres argumentos (deben estar en el orden correcto):<br>\
...
1. Añada el siguiente parámetro a la comunicación jvm que ejecuta:<br>\
...
4. Ejecutar `jconsole.exe` de JDK 1.6:<br>\
