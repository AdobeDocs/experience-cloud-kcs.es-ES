---
title: AEM "Problemas de configuración de GraphQl de la"
...
# AEM Problemas de configuración de GraphQl de
...
AEM AEM El cliente está desarrollando un sitio web que debe recuperar datos de sus instancias de mediante solución sin encabezado.

Para configurar el punto de conexión, consulte el tutorial en `[` 1`]`  se ha seguido.

Después de publicar todos los fragmentos de contenido, todos los modelos, contenidos, referencias, recursos y consultas tal como se describe en `[` 2`]` Sin embargo, al intentar recuperar cualquier fragmento de contenido de la instancia de publicación, se recibe un error 404 que impide que el contenido se lea desde el editor.
...
`[` 1`]`  aem_base_version: 6.5 aem_envId: xxxxxx aem_envType: dev aem_program_id: xxxxxx aem_release_id: xxxxx aem_service: cm-pxxxxx-exxxxxx aem_tenant: ns-team-aem-cm-prd-nxxxxx aem_tier: publish backend: publishfarm/- cache: <b>tapado</b>
clúster: `<` clúster`>`
código: - file_path: /var/log/aem/dispatcher.log host: publish-pxxxxxx-exxxxxxx.adobeaemcloud.com método: área de nombres del POST: ns-team-aem-cm-prd-nxxxxx orig_time: 25/jul/2023:15:39:11 +0000 ruta: /content/cq:graphql/graphql/endpoint.json nombre_pod: `<` pod_name`>`
pod_uid: `<` pod_uid`>`
