...
Este documento proporciona instrucciones sobre cómo se produce el vaciado y explica el mecanismo que ejecuta el vaciado y la invalidación de la caché.
<br>Cómo funciona<br>
<b>Orden de las operaciones</b>

El flujo de trabajo habitual se describe de la mejor manera cuando los autores de contenido activan una página, cuando el editor recibe el nuevo contenido, déclencheur una solicitud de vaciado para Dispatcher como se muestra en el siguiente diagrama:![](assets/___636e3fa8-7c2d-ef11-840b-6045bd006704___.png)

Esta sucesión de eventos demuestra que solo vaciamos elementos cuando son nuevos o han sido modificados.  Esto garantiza que el editor ha recibido el contenido antes de borrar la caché para evitar condiciones de carrera donde el vaciado podría ocurrir antes de que el editor recoja los cambios.
...
El primero es el agente de replicación estándar que envía las activaciones de contenido a.

El segundo es el agente inverso.  Este es opcional y está configurado para comprobar la bandeja de salida de cada editor con el fin de ver si hay contenido nuevo que introducir en la interfaz de autor como actividad de replicación inversa

#### AGENTE DE REPLICACIÓN DEL EDITOR

Esta es una captura de pantalla de ejemplo de un agente de replicación de vaciado estándar configurado:
...
En la pestaña de transporte, puede ver que el URI se ha configurado para que indique la dirección IP de Dispatcher que recibe las solicitudes de vaciado.  La ruta /dispatcher/invalidate.cache no es la manera en la que el módulo determina si hay vaciado. Solo es un punto final obvio que se puede ver en el registro de acceso para saber si se produjo una solicitud de vaciado.  En la pestaña Ampliado, revise las cosas que están ahí para certificar que se trata de una solicitud de vaciado para el módulo de Dispatcher.
...
- Host
  ...
  En la pestaña déclencheur, tome nota de los déclencheur alternados y utilice y cuáles son.


- <b>Ignorar predeterminado</b>

  Esto está habilitado para que el agente de replicación no se active cuando se activa una página.  Esto es algo que, cuando una instancia de autor tenía que realizar un cambio en una página, almacenaba en déclencheur un vaciado.  Como es un editor, no almacene en déclencheur ese tipo de evento.


- <b>Recepción activada</b>

  Cuando se reciba un nuevo archivo, vacíe la déclencheur.  Por lo tanto, cuando el autor nos envíe un archivo actualizado, almacene en déclencheur y envíe una solicitud de vaciado al distribuidor.


- <b>Sin versiones</b>

  Marque esta opción para evitar que el editor genere nuevas versiones porque se recibió un nuevo archivo.  Simplemente reemplace el archivo que tenemos y confíe en el autor para realizar un seguimiento de las versiones en lugar del editor.


Ahora, si observa el aspecto habitual de una solicitud de vaciado en forma de comando de flexión,
<br> <br>
...
#### Este diagrama muestra el aspecto de la estructura de este archivo cuando tiene elementos en la caché y el módulo del distribuidor ha enviado y procesado una solicitud de vaciado:
...
La configuración del nivel del archivo .stat indica cuántas carpetas profundas en el módulo atraviesa y actualiza un archivo .stat.  El archivo .stat está vacío. No es más que un nombre de archivo con una marca de fecha e incluso se podría crear manualmente ejecutando el comando táctil en la línea de comandos del servidor de Dispatcher.
...
En este ejemplo, utilice una configuración de nivel de archivo .stat de 4.  Esto garantiza que cuando vacíe contenido que se encuentra debajo de <b>us</b> que no hará que las carpetas de idioma se vacíen también.
...
AEM Como puede ver en este ejemplo, el archivo es más antiguo que el archivo .stat y se elimina, y se extrae uno nuevo de la caché para reemplazarlo en la caché antes de entregárselo al usuario final que lo solicitó.
<br>Configuración de archivo de granja<br>
...
Permite resaltar algunas de ellas que están relacionadas con el vaciado de caché
...
<br> 
Esta configuración indica cómo de profundos deben ser los archivos .stat cuando llega una solicitud de vaciado.
