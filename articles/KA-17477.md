...
- Si es un archivo de regla de lista blanca de IP, asígnele el nombre &quot;&lt;` description`>` _whitelist.rules` como el sufijo y la extensión. Esta convención de nombres le dará una descripción de para qué sirve y de que es un conjunto de reglas coincidentes de IP.
...
Si hubiera &quot;&lt;` FILENAME`>` _farm.any` archivos de `/etc/httpd/conf.dispatcher.d/available_farms/` directorio obtener enlace simbólico en el `/etc/httpd/conf.dispatcher.d/enabled_farms/` se utilizarán en la configuración en ejecución.
...
El ``<` FILENAME`>` _farm.any` files will have include statements for each file based on where they need to be included in the farm file. Here is an example syntax of a ``&lt;` FILENAME`>` _farm.any` como buena referencia:
